...
shaka.dash.DashParser.prototype.processManifest_ =
  ...
  if (this.config_.dash.maxSegmentDuration) {
    maxSegmentDuration = XmlUtils.parseDuration(
        this.config_.dash.maxSegmentDuration);
  }
  shaka.log.debug('maxSegmentDuration: ', maxSegmentDuration);
  ...
  if (this.manifest_) {
    ...
    var defaultPresentationDelay = Math.max(
        ...
        minBufferTime * 1.5);
    if (this.config_.dash.presentationDelay) {
      var configuredPresentationDelay =
          XmlUtils.parseDuration(this.config_.dash.presentationDelay);
      defaultPresentationDelay = configuredPresentationDelay ?
          configuredPresentationDelay : 0;
    }
    shaka.log.debug('Default presentation delay: ', defaultPresentationDelay);
